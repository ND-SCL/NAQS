...
class SimpleNet(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Linear(128 * 4 * 4)

    def forward(self, x, y=None):
        x = self.conv_pad_1(x)
        x = self.F.relu(self.conv_1(x))
        x = self.pool_1(self.pool_pad_1(x))
        x = self.drop_1(x)
        x = self.conv_pad_2(x)
        x = self.F.relu(self.conv_2(x))
        x = self.pool_2(self.pool_pad_2(x))
        x = self.drop_2(x)
        x = self.conv_pad_3(x)
        x = self.F.relu(self.conv_3(x))
        x = self.pool_3(self.pool_pad_3(x))
        x = self.drop_3(x)
        x = self.conv_pad_4(x)
        x = self.F.relu(self.conv_4(x))
        x = self.pool_4(self.pool_pad_4(x))
        x = self.drop_4(x)
        x = self.conv_pad_5(x)
        x = self.F.relu(self.conv_5(x))
        x = self.pool_5(self.pool_pad_5(x))
        x = self.drop_5(x)
        x = self.conv_pad_6(x)
        x = self.F.relu(self.conv_6(x))
