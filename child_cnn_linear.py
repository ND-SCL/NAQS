...
def build_layers(input_shape, num_classes, arch_paras):
    ...
    for paras in arch_paras:
        ...
        in_channels = out_channels
    num_features = compute_num_flat_features(
        input_shape, conv_layers, pool_layers, drop_layers)
    fc_layers = []
...
def compute_num_flat_features(input_shape, conv_layers,
                              pool_layers, drop_layers):
...
class CNN(nn.Module):
    ...
    def forward(self, x):
        x = conv_forward(x, self.conv_layers,
                         self.pool_layers, self.drop_layers)
        x = x.view(x.size()[0], -1)
...
class ChildCNN(object):
    ...
    def update_arch(self, arch_paras=[]):
        conv_layers, pool_layers, drop_layers, fc_layers = build_layers(
            self.input_shape, self.num_classes, arch_paras)
...
def quantize(x, num_int_bits=2, num_frac_bits=4):
    precision = 1 / 2 ** num_frac_bits
    bound = 2 ** num_int_bits
    return torch.clamp(
        torch.round(x/precision),
        min=0,
        max=bound-precision)


def compute_padding(height, width, kernel_height,
                    kernel_width, stride_height, stride_width):
    num_height_strides = math.floor((height - 1) / stride_height)
    num_height_paddings = num_height_strides * \
        stride_height + kernel_height - height
